...
def countFiles(path):
	"""Returns the number of files in the cwd and all of its subdirectories."""
	count = object
...
def findFiles(target, path):
	"""Returns a list of the filenames that contain the target
	string in the cwd and all its subdirectories."""
	files = []
	lyst = os.listdir(path)
	for element in lyst:
		if os.path.isfile(element):
			if target in element:
				files.append(path + os.set + element)
			else:
				os.chdir(element)
